...
- OpenAI Sites에 배포된 Jobvis Web의 정확한 HTTPS origin
- 인증서 갱신 알림을 받을 이메일
...
넣으면 자체 서명 인증서가 발급되어 브라우저와 Sites가 신뢰하지 않습니다.
Google·Kakao 로그인과 Gmail·Outlook·Calendar 연결은 각각 공급자 OAuth 앱 키가 있어야
활성화되지만, Sites 소유자 인증과 Jobvis 기본 기능에는 필요하지 않습니다.
...
./prepare-env.sh api.jobvis.example https://actual-sites-origin.example operator@example.com
./verify.sh
...
`prepare-env.sh`는 기존 `.env`를 덮어쓰지 않고 DB 비밀번호, 32바이트 AES 키, Sites 공유 비밀을
...
API `.env`의 아래 값과 Sites 프로젝트 설정을 정확히 맞춥니다.

| Sites 변수 | 값 |
| --- | --- |
| `JOBVIS_API_BASE_URL` | `https://<JOBVIS_API_HOST>` |
| `JOBVIS_API_MODE` | `sites` |
| `NEXT_PUBLIC_JOBVIS_API_MODE` | `sites` |
| `JOBVIS_TRUSTED_SITE_SECRET` | VM `.env`의 같은 이름 값 |
| `JOBVIS_WEB_ORIGIN` | 실제 Sites HTTPS origin |

API 쪽 `JOBVIS_WEB_ORIGIN`에는 Sites URL의 origin만 넣습니다. 경로와 마지막 `/`를 넣지 않습니다.
Sites URL이 바뀌면 `.env`의 `JOBVIS_WEB_ORIGIN`을 고친 뒤 다음 명령으로 API를 재생성합니다.

```bash
docker compose --env-file .env up -d --no-deps --force-recreate api
```

공유 비밀은 브라우저 공개 변수가 아니라 Sites 서버와 API에만 둡니다. Sites를 private
owner-only로 유지해야 플랫폼 사용자 헤더를 신뢰할 수 있습니다.

## 4. 외부 OAuth(선택)

`.env`에서 사용할 공급자 값만 채웁니다. 공급자 콘솔의 허용 redirect URI는 다음 하나입니다.

```text
https://<Sites origin>/oauth/callback
```

Google 로그인과 Gmail/Calendar 권한은 역할이 다르므로 각 기능에 필요한 client 설정을
확인합니다. 값 변경 후 API 컨테이너를 재생성하고 실제 연결·해제·재연결을 점검합니다.
...
정상이라고 배포 성공으로 판단하지 않습니다. 최소 smoke 순서는 Sites 로그인, 내 사용자 조회,
...
5. 소유한 hostname은 A/AAAA 레코드를 새 IP로 전환합니다. IP 기반 `sslip.io` hostname이면
   Sites의 `JOBVIS_API_BASE_URL`을 새 hostname으로 바꾸고 새 Sites version을 배포합니다.
6. 실제 공개 API hostname의 TLS readiness와 Sites 로그인·등록·조회 smoke scenario를
...
Sites API URL을 이전 주소로 되돌린 뒤 이전 API를 재시작합니다.

소유한 hostname과 `.env`를 유지하면 Web 설정과 사용자 식별자는 바뀌지 않습니다. IP가 이름에
포함된 `sslip.io` hostname을 썼다면 새 VM IP에 맞춘 hostname으로 API `.env`와 Sites의
`JOBVIS_API_BASE_URL`을 함께 바꾸고 Sites를 다시 배포합니다. DB dump만 옮기고 암호화 키를
